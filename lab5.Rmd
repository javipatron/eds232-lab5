...
```{r}
test<- data.frame(get_my_saved_tracks())
test2 <- get_my_saved_tracks()

class(test)

test$track.name
test$track.album.name
first <- test$track.artists
first[3,1]

test$track.id


join <- rbind(test,test2)
...
get_javier_tracks <- function(number) {
  ...
  returnValue(total_tracks)
}

get_javier_tracks(400)
